...
const getProducts = asyncHandler(async (req, res) => {
  ...
  const products = await Product.find({ ...keyword })
    .limit(pageSize) // 10 products per page
    .skip(pageSize * (page - 1)) // skip all products before current page
...
const getProductById = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    throw new Error('Product not found') // Error will be caught by global error handler
  }
})
...
// @access  Private
const createProductRrview = asyncHandler(async (req, res) => {
  const { rating, comment } = req.body

  const product = await Product.findById(req.params.id)

  if (product) {
    // check if the current has already reviewed this product
